...
public class BeVipRechargeViewHolder extends RecyclerView.ViewHolder {
    private Context context;
    private View itemView;
    //整个item
    @BindView(R.id.item_recharge_book_coin_ll)
    LinearLayout item_recharge_book_coin_ll;
    //充值金额
    @BindView(R.id.item_recharge_book_coin_money)
    TextView item_recharge_book_coin_money;
    //兑换的书币
    @BindView(R.id.item_recharge_book_coin_tv_coin)
    TextView item_recharge_book_coin_tv_coin;
    //赠送的书券
    @BindView(R.id.item_recharge_book_coin_tv_ticket)
    TextView item_recharge_book_coin_tv_ticket;
    //ishot
    @BindView(R.id.item_rech_book_iv_ishot)
    ImageView item_rech_book_iv_ishot;
    //赠送标志
    @BindView(R.id.item_recharge_book_coin_iv_zengsong)
    ImageView item_recharge_book_coin_iv_zengsong;
    //首充
    @BindView(R.id.item_recharge_book_coin_first)
    ImageView item_recharge_book_coin_first;
    private BeVipPresenter mPresenter;
    private ReadVipBean.GoodsListBean bean;
    private int position;
    private int selectedPosition = 0; //默认一个参数
    List<IsCheckBeanBeVip> isCheck = new ArrayList<>();
    IsCheckBean b = null;
//    private Map<Integer, Boolean> isClicks = new HashMap<>();//控件是否被点击,默认为false，如果被点击，改变值，控件根据值改变自身颜色

    public static BeVipRechargeViewHolder create(Context context, int type) {
        BeVipRechargeViewHolder imageViewHolder = new BeVipRechargeViewHolder(LayoutInflater.from(context).inflate(R.layout.item_recharge_book_coin, null), context);
        return imageViewHolder;
    }

    public BeVipRechargeViewHolder(@NonNull View itemView, Context context) {
        super(itemView);
    ...
    public void setData(BeVipPresenter mPresenter, ReadVipBean.GoodsListBean rechargeBookCoinBean, int position) {
        //选择哪一个
...
//
        ...
        if (position == IsCheckBeanBeVip.getPos()) {
